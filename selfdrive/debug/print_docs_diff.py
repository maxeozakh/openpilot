...
def extract_car_info_from_text(text):
  ...
  return matches.group(1).strip() if matches else "No car info found"
...
def compare_car_info(old_array, new_array):
  ...
  new_info_dict = {car.get('Model', 'Unknown Model'): car for car in new_array}
  ...
  for old_car in old_array:
    ...
    if model not in new_info_dict:
      ...
      if old_car.get('Detail sentence') != new_info_dict[model].get('Detail sentence'):
        changes["detail"].append(f"- Sentence for {model} changed!\n" +
                                  "  ```diff\n" +
...
def compare_car_dicts(old_car, new_car):
  return {key: value for key, value in old_car.items() if new_car.get(key) != value}
  ...
  for change_type, title in change_titles.items():
    if changes[change_type]:
      ...
      if change_type != "detail":
        ...
        for detail_change in changes["detail"]:
